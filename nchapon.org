...
** Font
Use Input Font (http://input.fontbureau.com/)  or Source code Pro font if present

#+BEGIN_SRC emacs-lisp
  (defun nc/setup-font ()
    "Set up font height"
    (interactive)
    (when (string= system-name "vagrant")
      (set-frame-font "Input Mono-12" nil t))
    (when (string= system-name "xps13")
      (set-frame-font "Input Mono-12" nil t)))
...
#+BEGIN_SRC emacs-lisp
  (setq org-agenda-files (list "~/notes/GTD" "~/notes/GTD/journal" "~/notes/projects")
        org-agenda-span 2
      ...
      (setq org-agenda-custom-commands
            '((" " "Agenda"
               ((agenda "" ((org-agenda-sorting-strategy '(timestamp-up time-up priority-down category-keep))))
                (tags-todo "TODO=\"STARTED\"+@cnp|TODO=\"WAITING\"+@cnp"
                ...
                (tags-todo "TODO=\"NEXT\"+@cnp"
                           ((org-agenda-overriding-header "Next tasks @office")
                            ...
                            (org-agenda-sorting-strategy '(priority-down todo-state-up category-keep))))
                (tags-todo "inbox|@home|@cnp+TODO=\"TODO\""
...
#+begin_src emacs-lisp
  (setq org-capture-templates
        '(("t" "todo" entry (file+headline (concat org-directory "GTD/mygtd.org") "Inbox" )
           "* TODO %?\n%U\n%a\n" :clock-in t :clock-resume t)
          ("n" "Next Task" entry (file+headline (concat org-directory "GTD/mygtd.org") "Inbox")
           "** NEXT %?")
          ("s" "Someday / Maybe" entry (file+headline (concat org-directory "GTD/mygtd.org") "Someday / Maybe")
            "* SOMEDAY %? :IDEA:\n%u" :clock-in t :clock-resume t)
          ("f" "FishLog" plain (file+datetree+prompt (concat org-directory "projects/fishing.org"))
           "%[~/notes/templates/fishlog.org]")
          ("F" "Film" entry (file+headline (concat org-directory "GTD/watching.org") "Films à voir")
               "* NEXT %^{Titre}
       %i
       - *Réalisateur:* %^{Auteur}
       - *Année:* %^{année}
      ...
      %U" :prepend t)
          ...
          ("i" "Interrupting task" entry
             (file+datetree (concat org-directory "GTD/diary-log.org"))
             "* %^{Task}"
             :clock-in t :clock-resume t)
          ("m" "Meeting" entry (file+headline (concat org-directory "GTD/calendar.org") "Réunions")
           "* MEETING %? :MEETING:\n%U\nSCHEDULED: %(org-insert-time-stamp (org-read-date nil t \"+0d\"))\n")
          ("p" "Phone call" entry (file+datetree (concat org-directory "GTD/diary-log.org"))
                 "* PHONE %? :PHONE:\n%U" :clock-in t :clock-resume t)
          ("a" "RendezVous" entry (file+headline (concat org-directory "GTD/calendar.org") "RendezVous")
                 "* RDV %? :APPT:\n%U\nSCHEDULED: %(org-insert-time-stamp (org-read-date nil t \"+0d\"))\n")
          ("j" "Journal" entry (file+datetree org-default-notes-file)
           "* %?\nEntered on %U\n  %i\n  %a")
          ("w" "WeeklyReview" entry (file+datetree+prompt (concat org-directory "GTD/weekly-review.org"))
           "* Summary of the week :REVIEW:\n%[~/notes/templates/review.org]")
           ("D" "Done Business Task" entry
             (file+headline (concat org-directory "GTD/mygtd.org") "Done Tasks")
             "* DONE %^{Task}"
             :clock-in t :clock-resume t)
          ))
#+end_src
...
#+begin_src emacs-lisp
  (use-package eshell
    ...
    (defun nc--eshell-buffer-name (parent)
        "Create eshell buffer name from PARENT directory."

        (let* ((p-lst (split-string parent "/"))
               (name  (mapconcat (lambda (elm) elm) (last p-lst 3) "/")))
        (concat "*eshell: " name "*")))

    (defun nc--eshell-chdir(parent)
      (let ((b-name (nc--eshell-buffer-name parent)))
  ...
  directory to make multiple eshell windows easier."
      ...
      (let* ((parent (if (buffer-file-name)
                       ...
                       default-directory))
             (b-name (nc--eshell-buffer-name parent)))

        (cond ((get-buffer b-name)
               (switch-to-buffer-other-window b-name))
              (t (nc--eshell-chdir parent)))))

    (require 'em-term)
    (setq eshell-visual-commands '("vi" "screen" "top" "less" "more" "lynx"
                                   "ncftp" "pine" "tin" "trn" "elm" "htop" "node" "npm")
          eshell-visual-subcommands '(("git" "log" "diff" "show"))
          eshell-aliases-file (concat no-littering-etc-directory ".eshell-aliases")))
#+end_src
