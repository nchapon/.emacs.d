...
*** Key Chords
 #+BEGIN_SRC emacs-lisp
   (use-package key-chord
     ...
     (key-chord-define-global "--" 'org-ctrl-c-minus)
     (key-chord-define-global "**" 'org-ctrl-c-star)
...
*** Files and directories
 ...
 #+begin_src emacs-lisp
   (defconst nc/org-default-projects-dir (concat org-directory "gtd"))
   (defconst nc/org-default-projects-file (concat org-directory "gtd/projects.org"))
   ...
   (defconst nc/org-default-completed-dir (concat org-directory "gtd/_completed"))
   (defconst nc/org-journal-dir (concat org-directory "journal"))
   (defconst nc/inbox-file (concat org-directory "gtd/inbox.org"))
   (defconst nc/org-default-inbox-file (concat org-directory "gtd/inbox.org"))
...
#+BEGIN_SRC emacs-lisp
  (defun nc/projects-overview ()
    (interactive)
    (org-agenda "project" "m"))
#+END_SRC
...
*** Refiling
 ...
 #+begin_src emacs-lisp
   ...
   (setq org-refile-targets (append '((nc/org-default-projects-file :level . 1)
                                      (org-default-notes-file :level . 0)
                                      (nil :maxlevel . 1))
                                      (->>
                                       (directory-files nc/org-default-projects-dir nil ".org$")
                                       (-remove-item (file-name-base nc/org-default-projects-file))
                                       (--remove (s-starts-with? "." it))
                                       (--map (format "%s/%s" nc/org-default-projects-dir it))
...
#+END_SRC
 ...
 #+BEGIN_SRC emacs-lisp
   (setq org-agenda-files (list "~/notes/gtd/" "~/notes/personal/calendar.org" "~/notes/journal/")
         org-agenda-span 'day
                      ...
                      ((agenda "" ((org-agenda-sorting-strategy '(timestamp-up time-up priority-down category-keep))))
                       ...
                       (tags-todo "TODO=\"TODO\"|TODO=\"STARTED\"-@office"
                                  ((org-agenda-overriding-header "Other tasks")
                                   ...
                                   (org-agenda-files '("~/notes/gtd/inbox.org"))
                                   (org-agenda-sorting-strategy '(priority-down todo-state-up category-keep))))
                       )
                      nil)
               ("g" . "GTD contexts")
...
**** Recent open loops
 ...
 #+END_SRC
*** Refiling subtree to new file
From https://gitlab.com/howardabrams/spacemacs.d/-/blob/master/elisp/boxes-extras.el
Documentation :
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun org-refile-to-projects-dir ()
    ...
    (interactive)
    (org-refile-subtree-to-file nc/org-default-projects-dir))
...
=F1= will invoke hydra context laucher, if there is no hydra for the
current context, it display by default global hydra
  ...
  (defun nc/context-hydra-launcher ()
    ...
    (cl-case major-mode
      ('org-mode (let* ((elem (org-element-context))
           ...
           (cl-case etype
             (src-block (hydra-org-babel/body))
             ((table-row table-cell) (hydra-org-table/body) )
             (t (hydra-org/body))))
             )
...
#+BEGIN_SRC emacs-lisp
  (setq nc--f-key-settings (concat
    ...
    (propertize
    "F1      S-F1            F2        | F5      F6     F7       | F8    F9                  F10      F11     F12
  "
  'face '(:foreground "teal"))
    "Hydra   Hydra Global    Treemacs  | Revert  Magit  Yankpad  | Deft  Interruption task   Menu     Clock   Agenda
  "
    "---------------------------------------------------------------------------------------------------------------"))
#+END_SRC

*** Hydra Global -> Shift F1
...
")
    ...
    ("e" nc/find-config "emacs" :color blue :column "Go To")
    ("I" nc/go-to-inbox "inbox tasks file")
    ...
    ("P" (lambda ()
       ...
       ) "Projects" :color blue :column "Directories")
    ("R" (lambda ()
               (interactive)
               (dired nc/org-default-resources-dir)
       ) "Resources")
    ...
    ;; Insert
    ("ip" nc/generate-password "password"  :column "Inserting")
    ("iD" nc/insert-datestamp "datestamp")
    ("id" nc/insert-datestamp-inactive "date stamp inactive")
    ;; Actions
    ("d" nc/delete-this-file "delete file"  :column "ACTIONS")
    ("r" nc/rename-this-file-and-buffer "rename file")
    ("s" helm-do-ag "search (ag)")

    ;; Quit
    ("q" nil "quit" :column nil :color red))

  (define-key global-map [(shift f1)] 'hydra-global/body)
#+END_SRC
*** Hydra org
 #+BEGIN_SRC emacs-lisp
   (defhydra hydra-org
...
")
     ...
     ("I" nc/go-to-inbox "inbox tasks file" :color blue :column "Org Files")
     ("j" nc/journal-file-today "journal file today")
     ("y" nc/journal-file-yesterday "yesterday file")
     ("p" nc/projects-overview "Projects overview")

     ("P" (lambda ()
            (interactive)
            (dired nc/org-default-projects-dir)
            ) "Projects" :color blue :column "Directories")
     ("R" nc/go-to-resources-dir "Resource")
     ("P" (lambda ()
            (interactive)
    ...
    ("X" (lambda ()
     ...
     ("t" nc/insert-time-slot "time slot" :column "Inserting")
     ("d" nc/journal-file-insert "daily template")
     ("D" nc/daily-review "daily review")
     ("iD" nc/insert-datestamp "datestamp")
     ("id" nc/insert-datestamp-inactive "date stamp inactive")

     ("2" org-tags-view "org-tags-view" :column "Actions")
     ("3" org-tags-sparse-tree "org-tags-sparse-tree")
